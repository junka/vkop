...
void Add::submit(int out_width, int out_height) {
    ...
    VulkanPipeline pipeline(device, types, objs,
                            reinterpret_cast<const uint32_t *>(add_spv),
                            add_spv_len);
...
void Add::execute(std::vector<std::shared_ptr<core::Tensor<float>>> inputs,
                  std::vector<std::shared_ptr<core::Tensor<float>>> outputs) {
    apply<float>(inputs, outputs);
}

void Add::execute(std::vector<std::shared_ptr<core::Tensor<int>>> inputs,
