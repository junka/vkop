...
class YOLO11:
    ...
    def postprocess(self, input_image: np.ndarray, output: list[np.ndarray], pad: tuple[int, int]) -> np.ndarray:
        ...
        for i in np.array(indices).flatten():
            ...
            print(f"Detection: Class {class_id}, Score: {score:.2f}, Box: {box}, image width: {input_image.shape[1]}, image height: {input_image.shape[0]}")
    ...
    def modify_onnx_model(self, intermediate_tensor_name: str) -> None:
        """Modify the ONNX model to expose the intermediate tensor."""
        model = onnx.load(self.onnx_model)

        # Check if the tensor already exists in outputs
        existing_outputs = [output.name for output in model.graph.output]
        if intermediate_tensor_name not in existing_outputs:
            model.graph.output.extend([onnx.ValueInfoProto(name=intermediate_tensor_name)])
            onnx.save(model, "modified_model.onnx")
            print(f"Modified model saved as 'modified_model.onnx' with intermediate output '{intermediate_tensor_name}'.")
        else:
            print(f"Tensor '{intermediate_tensor_name}' is already exposed as an output.")

    def main(self, intermedia_tensor_name) -> np.ndarray:
        ...
        if intermedia_tensor_name is not None:
            self.modify_onnx_model(intermedia_tensor_name)
            session = ort.InferenceSession("modified_model.onnx", providers=providers or available, sess_options=so)
        else:
            session = ort.InferenceSession(self.onnx_model, providers=providers or available, sess_options=so)
        ...
        print("Model input shape:", model_inputs[0].shape)
        print("pad values:", pad)
        if intermedia_tensor_name is not None:
            outputs, mideia = session.run([session.get_outputs()[0].name, intermedia_tensor_name], {model_inputs[0].name: img_data})
            print("Intermediate tensor shape:", mideia.shape)
            print("Intermediate tensor values:", mideia)
        else:
            outputs = session.run([], {model_inputs[0].name: img_data})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img", type=str, default=str("dog.jpeg"), help="Path to input image.")
    parser.add_argument("--conf-thres", type=float, default=0.5, help="Confidence threshold")
    parser.add_argument("--iou-thres", type=float, default=0.5, help="NMS IoU threshold")
    parser.add_argument("--intermedia", type=str, help="Name of the intermediate tensor to be exposed")
    args = parser.parse_args()

    # Check the requirements and select the appropriate backend (CPU or GPU)
    check_requirements("onnxruntime-gpu" if torch.cuda.is_available() else "onnxruntime")

    # Create an instance of the YOLO11 class with the specified arguments
    detection = YOLO11(args.model, args.img, args.conf_thres, args.iou_thres)

    # Perform object detection and obtain the output image
    output_image = detection.main(args.intermedia)
