...
class ONNXOptimizer:
    """
    Optimize ONNX model using ONNX's built-in optimizer.
    """
    @staticmethod
                    ...
                    gemm_node = {
                        ...
                        'inputs': [
                            gap_node['outputs'][0],  # Output from GlobalAveragePool (becomes matrix A in GEMM)
                            {
                                ...
                                'shape': [original_shape[0], original_shape[1]] if 'original_shape' in locals() else weight_input['shape'][:2]
            ...
            if sorted(normalized_axes) == [2, 3]:
                ...
                if keepdims == 1:
                    ...
                    if (len(output_after_reduce) == 4 and 
                        ...
                        print(f"Found ReduceMean '{node['name']}' with axes {axes} (normalized to {normalized_axes}) followed by Reshape '{reshape_node_dict['name']}', replacing with GlobalAveragePool")
                        ...
                        if reshape_idx is not None:
                            ...
                            globalavgpool_node = {
                                ...
                                'outputs': [{
                                    'name': reshape_node_dict['outputs'][0]['name'],  # Output from Reshape
...
class ModelConverter:
    ...
    def parse_onnx_model(onnx_path, batch_size):
        ...
        model = ONNXOptimizer.optimize_onnx_model(model, batch_size)
        ...
        if ONNXOptimizer.is_topologically_sortable(graph) == False:
    ...
    def apply_optimizations(self, dag_model, args):
        # Apply optimizations
        InitializerMerger.move_input_tensor_to_attr(dag_model)
        InitializerMerger.merge_initializers(dag_model)
        InitializerMerger.convert_flat_to_reshape(dag_model)
        InitializerMerger.remove_redundant_reshape(dag_model)
        FusionOptimizer.fuse_conv_bn(dag_model)
        FusionOptimizer.fuse_gated_conv(dag_model)
        FusionOptimizer.fuse_conv_simple_activation(dag_model)
        FusionOptimizer.replace_reducemean_reshape_with_globalaveragepool(dag_model)
        FusionOptimizer.unify_reduce_operators(dag_model)
        FusionOptimizer.replace_globalaveragepool_conv_with_gemm(dag_model)

        dag_model.build_dependencies()

        if args.quant == "fp16":
            Quantizer.quantize_to_fp16_selective(dag_model)
        elif args.quant == "int8":
            Quantizer.quantize_to_int8_weight_only(dag_model)
...
def unified_initializers(vk_model):
    ...
    initializer_consumers = defaultdict(list)
    for node in vk_model.nodes.values():  # DAGBasedModel.nodes is a dict
        for inp in node.inputs:
            initializer_consumers[inp['name']].append(node)
...
def convert_initializers_nchw_to_rgba(vk_model):
    ...
    for name, initializer in list(vk_model.initializers.items()):  # Use list() to avoid dict changing during iteration
        dims = list(initializer.dims)

        # Skip if this is metadata from previous runs
        if name.startswith(('unified_', 'rgba_conversion_')):
            continue
        ...
        if len(dims) in [3, 4]:
            ...
            converted_count += 1
            print(f"Converted {name} from shape {dims} to RGBA format len {rgba_flat.shape} with shape {padded_dims}")
...
def main():
    ...
    convertor = ModelConverter()
    dag_model = convertor.parse_onnx_model(args.input, batch_size)

    convertor.apply_optimizations(dag_model, args)

    if args.unify:
        unified_initializers(dag_model)
    if args.rgba:
        convert_initializers_nchw_to_rgba(dag_model)
