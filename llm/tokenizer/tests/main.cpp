...
// 极简 JSON 解析：只读 tests/hf_ground_truth.json 的 [{s, ids}, ...] 结构。
...
int main() {
    try {
        ...
        // ===== 测试 5：HF 一致性（核心）=====
        {
            auto cases = load_ground_truth("tests/hf_ground_truth.json");
            assert(!cases.empty() && "ground truth fixture missing");
            int diffs = 0;
